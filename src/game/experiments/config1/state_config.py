...
class BiddingState:
    ...
    def get_legal_actions(self):
        ...
        if   self.player_turn == "M": threshold = None
        elif self.player_turn == "F": threshold = 27
        elif self.player_turn == "R": threshold = 40
        else:                         threshold = None
    ...
    def apply(self, action):
        ...
        if self.players_in:
            if self.player_turn in self.players_in:
                idx = self.players_in.index(self.player_turn)
            else:
                idx = 0
            self.player_turn = self.players_in[(idx + 1) % len(self.players_in)]
        return self
...
def interactive_bidding_phase(hand_F, hand_M, hand_R, iterations=2000):
    """
    In config1, Middlehand (M), Forehand (F), and Rearhand (R) all use MCTS bidding.
    This mirrors ai_bidding_phase but prints each AI decision for debugging/observation.
    """
    print("=== Skat Bidding Phase (AI-driven) ===\n")

    # --- Round 1: Middlehand (M) vs Forehand (F) ---
    print("--- Round 1: Middlehand (M) vs Forehand (F) ---")
    state = BiddingState(
        current_bid=18,
        player_turn="M",
        players_in=["M", "F"],
    ...
    while set(state.players_in) == {"M", "F"}:
        actor = state.player_turn
        action = mcts_bidding(state, iterations=iterations)
        print(f"{actor} plays: {action}")
        state.apply(action)

    round1_winner = state.players_in[0]
    print(f"\nRound 1 winner: {round1_winner} (bid = {state.current_bid})\n")

    # --- Round 2: Winner vs Rearhand (R) ---
    print(f"--- Round 2: {round1_winner} vs Rearhand (R) ---")
    state2 = BiddingState(
    ...
    while len(state2.players_in) > 1:
        actor = state2.player_turn
        action = mcts_bidding(state2, iterations=iterations)
        print(f"{actor} plays: {action}")
        state2.apply(action)

    final_winner = state2.players_in[0]
    final_bid    = state2.current_bid
    print(f"\nFinal winner of bidding: {final_winner} (bid = {final_bid})\n")
