...
import time
import numpy as np
from src.game.experiments.config2.trick_phase_config2 import Trick, TrickTakingState, mcts_trick_phase
...
def run_ui(hand_F, hand_M, hand_R, skat_cards,
    ...
    while running:
        ...
        if state.current_player == "M":
            # Player 3 (MCTS only, like Player 1)
            t0 = time.perf_counter()
            c3 = mcts_trick_phase(state, iterations=2000)
    ...
    for label,pid in [("Player 1","F"), ("Player 2","R"), ("Player 3","M")]:
        total_time = times[pid] + counts[pid] * 1.0
